// Package ginapi provides primitives to interact with the openapi HTTP API.
//
...
type ServerInterface interface {
	// Lock a resource.
	// (POST /locks)
	PostLocks(c *gin.Context, params PostLocksParams)
	// Unlock a resource.
	// (DELETE /locks/{resource_id})
	DeleteLocksResourceId(c *gin.Context, resourceId ResourceId, params DeleteLocksResourceIdParams)
}
...
func (siw *ServerInterfaceWrapper) PostLocks(c *gin.Context) {
	...
	if paramValue := c.Query("resource_id"); paramValue != "" {
		...
		siw.ErrorHandler(c, fmt.Errorf("Query argument resource_id is required, but not found"), http.StatusBadRequest)
		return
	}

	err = runtime.BindQueryParameter("form", true, true, "resource_id", c.Request.URL.Query(), &params.ResourceId)
	if err != nil {
		siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter resource_id: %s", err), http.StatusBadRequest)
		return
	}
	...
	siw.Handler.PostLocks(c, params)
}

// DeleteLocksResourceId operation middleware
func (siw *ServerInterfaceWrapper) DeleteLocksResourceId(c *gin.Context) {
	...
	err = runtime.BindStyledParameter("simple", false, "resource_id", c.Param("resource_id"), &resourceId)
	if err != nil {
		siw.ErrorHandler(c, fmt.Errorf("Invalid format for parameter resource_id: %s", err), http.StatusBadRequest)
		return
	}
	...
	siw.Handler.DeleteLocksResourceId(c, resourceId, params)
}
